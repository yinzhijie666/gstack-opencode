...
| `/review` | `shipped-v1` | `test/opencode-review-smoke.test.ts`, `test/opencode-review-fix-smoke.test.ts` | report-first by default, supports explicit low-risk fix pass |
| `/qa` | `shipped-v1` | `test/opencode-qa-smoke.test.ts`, `test/opencode-qa-fix-smoke.test.ts` | report-first by default, supports explicit bounded fix loop |
| `/qa-only` | `shipped` | `test/opencode-qa-only-smoke.test.ts` | explicit report-only QA path |
| `/design-review` | `shipped-v1` | `test/opencode-design-review-smoke.test.ts`, `test/opencode-design-review-fix-smoke.test.ts` | audit-first by default, supports explicit bounded UI fix path |
| `/ship` | `shipped-v1` | `test/opencode-ship-smoke.test.ts`, `test/opencode-ship-commit-smoke.test.ts`, `test/opencode-ship-push-smoke.test.ts` | local-first by default, supports explicit commit and push paths |
...
- `/qa`
  - regression generation and richer fix-loop behavior still need more depth
- `/design-review`
  - fix mode exists, but broader redesign and richer before/after automation are still narrower than upstream ambition
- `/ship`
  - commit path is verified, but push/PR creation parity is still shallower than the strongest upstream path
- `/document-release`
